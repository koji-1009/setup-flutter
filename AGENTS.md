...
* **Vitest runs with its default mock options**: Only `clearMocks` is on, so call history is cleared before each test but implementations, `vi.spyOn` spies and `vi.stubEnv` values persist. Undo them in `afterEach` (`vi.restoreAllMocks()`, `vi.unstubAllEnvs()`), and wrap `vi.useFakeTimers()` in `try/finally` with `vi.useRealTimers()`.
