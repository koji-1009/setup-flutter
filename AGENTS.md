...
* **Test state is reset before every test**: `vitest.config.mts` sets `mockReset`, `restoreMocks` and `unstubEnvs`, so mock implementations, `vi.spyOn` spies and `vi.stubEnv` values never carry over; do not add manual cleanup for them. Give a `vi.mock` factory's default behaviour as `vi.fn(impl)`; a `vi.fn().mockResolvedValue(...)` chain does not survive the reset. Fake timers have no such option: wrap `vi.useFakeTimers()` in `try/finally` with `vi.useRealTimers()`.
