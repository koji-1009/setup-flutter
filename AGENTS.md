...
* **ESM mocking**: Use `vi.mock()` + `Mocked<typeof>`, not `vi.spyOn()` on module namespaces. A class passed to `mockImplementation()` must define its members as class fields (`get = vi.fn()...`); prototype methods on it are not reachable from the created instance. Automocked classes (bare `vi.mock()`) do expose prototype methods as mocks.
* **Mocks are reset before every test**: `vitest.config.mts` sets `mockReset: true`, so each mock's implementation goes back to its initial one and no test sees a return value set by another. Give a `vi.mock` factory's default behaviour as `vi.fn(impl)`; a `vi.fn().mockResolvedValue(...)` chain does not survive the reset. `vi.spyOn` spies and `vi.stubEnv` values are not covered: undo them in `afterEach` (`vi.restoreAllMocks()`, `vi.unstubAllEnvs()`), and wrap `vi.useFakeTimers()` in `try/finally` with `vi.useRealTimers()`.
